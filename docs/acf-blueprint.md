...
- 基础 GUI 运营控制台
- 基于当前仓库 runtime 的可接入适配层，并为后续 `CC/Codex` 代理 Agent 预留边界
...
- 以本地优先控制平面为中心
- 以当前仓库作为原生执行引擎与宿主
- 以后续 `CC/Codex` 代理 Agent 扩展为增量能力，而不是首期前提
...
| --- | --- | --- | --- |
| Agent runtime | 当前仓库已经具备多 Agent 执行、工具调用和后台任务循环 | 作为原生执行引擎复用 | 高可行 |
...
- 首期先跑通原生执行引擎闭环，后续再接入 `CC/Codex` 代理 Agent
...
- `BMAD` 进入方法标准包和首期轻量机制
- 两者都不直接替代控制平面对象层和执行引擎

### 4.7 可行性结论

综合判断：

- 当前仓库足以作为 `ACF` 的原生执行引擎与交互壳
...
- 以原生引擎优先为原则，先在当前仓库内跑通闭环
- 以代理 Agent 后置为原则，为后续 `CC/Codex` 接入预留边界
...
控制平面负责业务对象、阶段裁决和治理状态；原生执行引擎负责执行。二者分层后：
...
- 与其他对象的关系：被任务调度和前台入口引用；底层可映射到现有 markdown agent 能力，并为后续代理 Agent 映射预留字段
- 首期是否必须落库：是
...
| `prompt_source` | Prompt 来源 | `enum` | 是 | system | `markdown_agent/control_plane_profile/proxy_agent_profile` |
...
| `9.3` 连接底层 agent 文件能力 | 将控制平面配置映射到现有 markdown agent 基座，并为代理 Agent 预留扩展边界 |
...
- 当前仓库继续作为执行内核与宿主，不重写底层 runtime。
- `CC/Codex` 作为后续代理 Agent 形态接入，不属于首期闭环前提。
...
基于当前仓库继续开发 `ACF` 是可行的，但前提是边界必须保持清晰：不能把执行基座误写成控制平面，不能把首期 MVP 和完整路线图混为一体，不能把需求、验证、缺陷和 Gate 只留在聊天上下文里，也不能忽略控制平面与代码版本的锚定关系。同时，后续 `CC/Codex` 代理 Agent 接入应建立在首期原生闭环稳定之后。
