...
| `02-runtime-adapter` | 建立控制平面到当前原生执行引擎的接入层，并为后续代理 Agent 预留边界 | 运行时适配、事件投影、调度适配 | 01 |
| `03-intake-and-start` | 建立需求入口、项目画像和 `Start` 冻结机制 | 草稿入口、上下文摘要、readiness 检查、变更分流 | 01, 02 |
